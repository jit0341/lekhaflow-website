    ...
    <AnimatePresence>
      {isOpen && (
        <motion.div
          ...
          animate={{ opacity: 1 }}
          exit={{ opacity: 0 }}
          className="fixed inset-0 z-[200] flex items-center justify-center bg-black/50 backdrop-blur-xl p-4"
        >
          <motion.div
            initial={{ scale: 0.9, y: 20 }}
            animate={{ scale: 1, y: 0 }}
            className="bg-white border-2 border-teal-500 p-8 md:p-10 rounded-[3rem] max-w-lg w-full relative shadow-2xl max-h-[90vh] overflow-y-auto"
          >
            <button
              onClick={onClose}
            ...
            {step === "form" && (
              <>
                <div className="text-center mb-8">
                  ...
                  <p className="text-teal-600 font-black text-3xl tracking-tighter mt-2">₹{price}</p>
                  <p className="text-slate-500 text-[10px] font-bold uppercase tracking-widest mt-1">Yearly Subscription</p>
                </div>

                <form onSubmit={handleSubmit} className="space-y-4">
                  <div>
                    <label className="text-[10px] font-black text-teal-600 uppercase tracking-widest block mb-2">Full Name *</label>
                    <input
                      required
                      type="text"
                      placeholder="Jitendra Bharti"
                      className="w-full bg-slate-50 border border-slate-200 p-4 rounded-2xl text-slate-900 font-bold text-xs outline-none focus:border-teal-500 transition-all"
                      value={formData.name}
                      onChange={(e) => setFormData({ ...formData, name: e.target.value })}
                    />
                  </div>
                  <div>
                    <label className="text-[10px] font-black text-teal-600 uppercase tracking-widest block mb-2">Company / Firm Name *</label>
                    <input
                      required
                      type="text"
                  ...
                  <div>
                    <label className="text-[10px] font-black text-teal-600 uppercase tracking-widest block mb-2">Email *</label>
                    <input
                      required
                      type="email"
                  ...
                  <div>
                    <label className="text-[10px] font-black text-teal-600 uppercase tracking-widest block mb-2">WhatsApp Number *</label>
                    <input
                      required
                      type="tel"
            ...
            {step === "payment" && (
              <>
                <div className="text-center mb-8">
                  <h3 className="text-2xl font-black text-slate-900 uppercase tracking-tighter italic">Complete Payment</h3>
                  <p className="text-slate-500 text-xs mt-2">Your details have been saved. Choose a payment method:</p>
                </div>
                ...
                <div className="space-y-4">
                  <a
                    ...
                    className="flex items-center gap-4 w-full p-5 bg-slate-50 border border-slate-200 rounded-2xl hover:border-teal-500 transition-all group"
                  >
                    ...
                    <div className="text-left">
                      <p className="text-slate-900 font-black text-sm uppercase tracking-widest">Pay via Razorpay</p>
                      <p className="text-slate-500 text-[10px]">UPI, Card, NetBanking, Wallet</p>
                    </div>
                  </a>

                  <div className="flex items-center gap-4 w-full p-5 bg-slate-50 border border-slate-200 rounded-2xl">
                    <div className="w-12 h-12 bg-amber-50 rounded-xl flex items-center justify-center">
                      <Smartphone className="text-amber-600" size={24} />
                    </div>
                    <div className="text-left">
                      <p className="text-slate-900 font-black text-sm uppercase tracking-widest">UPI / Bank Transfer</p>
                  ...
                  <div className="flex items-center gap-4 w-full p-5 bg-slate-50 border border-slate-200 rounded-2xl">
                    ...
                    <div className="text-left">
                      <p className="text-slate-900 font-black text-sm uppercase tracking-widest">NEFT / IMPS</p>
                ...
                <div className="mt-6 p-4 bg-amber-50 border border-amber-200 rounded-2xl">
                  <p className="text-amber-700 text-[10px] font-bold uppercase tracking-widest text-center">
                ...
                <button
                  ...
                  className="w-full mt-4 py-4 bg-slate-900 text-white font-black rounded-2xl uppercase text-[10px] tracking-widest hover:bg-slate-800 transition-all"
                >
            ...
            {step === "success" && (
              <div className="text-center py-10">
                <CheckCircle2 size={64} className="text-teal-600 mx-auto mb-6" />
                <h3 className="text-2xl font-black text-slate-900 uppercase tracking-tighter italic mb-4">License Generated!</h3>
                ...
                <div className="bg-slate-50 border border-slate-200 rounded-2xl p-6 mb-6">
                  ...
                  <ol className="text-left text-slate-600 text-xs space-y-2">
