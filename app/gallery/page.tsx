...
export default function Gallery() {
  ...
  return (
    <div className="min-h-screen bg-white text-slate-100 font-sans tracking-tight text-xs antialiased p-6">
      
      {/* HEADER SECTION */}
      <div className="max-w-6xl mx-auto flex justify-between items-center border-b border-slate-200 pb-6 mb-12">
        <div>
          <Link href="/">
            <div className="text-xl font-black text-slate-950 tracking-wider cursor-pointer mb-1">
        ...
        <Link href="/">
          <span className="px-4 py-2 rounded-xl bg-white border border-slate-200 text-slate-950 hover:bg-slate-800 hover:text-amber-700 font-black transition cursor-pointer text-[10px] uppercase tracking-wider">
      ...
      <main className="max-w-6xl mx-auto space-y-12">
        <div className="text-center max-w-xl mx-auto space-y-2">
          <span className="inline-block bg-amber-500/10 text-amber-700 text-[9px] font-black uppercase tracking-widest px-2.5 py-1 rounded border border-amber-500/20 font-mono">
            Visual Telemetry Assets
          </span>
          <h1 className="text-2xl font-black text-slate-950 tracking-tight">Verified Automation Infrastructure Screens</h1>
        ...
        <div className="grid md:grid-cols-2 gap-8 pt-4">
          {screenshots.map((screen) => (
            <div key={screen.id} className="bg-gradient-to-b from-slate-900 to-white border-2 border-slate-200 rounded-2xl overflow-hidden shadow-2xl flex flex-col group hover:border-amber-500/40 transition-all duration-300">
              
              {/* IMAGE CONTAINER ACCEPTS REAL IMAGE LINK */}
              <div className="relative aspect-video w-full bg-white border-b border-slate-200 flex items-center justify-center overflow-hidden">
                ...
                <div className="z-10 w-[90%] h-[80%] bg-white/90 border border-slate-300/60 rounded-lg shadow-2xl p-4 font-mono text-[9px] text-slate-600 flex flex-col justify-between select-none">
                  <div className="flex justify-between items-center border-b border-slate-200 pb-1.5 mb-2">
                    ...
                    <span className="text-[8px] bg-white px-1.5 py-0.5 rounded border border-slate-200 text-amber-700 font-black font-mono">{screen.tag}</span>
                  </div>
                  <div className="flex-1 font-mono text-[8px] space-y-1 text-slate-600 leading-normal">
                    ...
                    <p className="text-amber-700 font-mono">[WARN] Cryptographic Machine Lock ID Verified.</p>
                  </div>
              ...
              <div className="p-5 space-y-2 flex-1 flex flex-col justify-between">
                <div className="space-y-1">
                  <div className="flex justify-between items-center">
                    <h3 className="text-sm font-black text-slate-950 tracking-wide group-hover:text-amber-700 transition-colors">
                      {screen.title}
                    </h3>
                    <span className="bg-white border border-slate-200 text-slate-600 px-2 py-0.5 rounded font-mono text-[8px] font-black uppercase">
                      Node 0{screen.id}
                    </span>
                  </div>
                  <p className="text-slate-600 text-[11px] font-medium leading-relaxed font-sans">
                    {screen.description}
                  </p>
                </div>
              </div>
        ...
        <div className="bg-slate-50 border border-slate-200 p-6 rounded-2xl text-center max-w-2xl mx-auto space-y-4">
          <div className="text-xs font-black text-slate-950">Want to experience these dashboard operations in realtime on your system?</div>
          <p className="text-slate-600 text-[11px] max-w-md mx-auto font-medium font-sans">
